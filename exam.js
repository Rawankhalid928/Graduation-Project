...
function selectAnswer(index) {
    ...
    // تحديث الأزرار لتغيير التلوين
    updateQuestion();

    // إضافة علامة الصح بجانب رقم السؤال في القائمة الجانبية
    document.getElementById("qbtn" + currentQuestion).classList.add("answered");
}
...
function updateQuestion() {
    let q = [...theoryQuestions, ...practicalQuestions][currentQuestion - 1];

    // عرض السؤال
    document.getElementById("question-text").innerHTML = `سؤال ${currentQuestion}: ${q.question}`;
    ...
    // زر إنهاء الاختبار يظهر فقط عند آخر سؤال
    document.getElementById("finishBtn").style.display = (currentQuestion === totalQuestions) ? "inline-block" : "none";

    // تلوين زر السؤال الحالي في القائمة الجانبية
    document.querySelectorAll("#question-nav button").forEach(btn => btn.classList.remove("current"));
    document.getElementById("qbtn" + currentQuestion).classList.add("current");
}



function finishQuiz() {
    let score = Object.keys(answers).length;
    document.getElementById("resultBox").innerText = `لقد حصلت على ${Math.round((score / totalQuestions) * 100)} من 100`;
}
...
{
        question:"ماهي مخرجات الكود التالي <pre><code> < ?php <br> $array[0]='Sara';<br> $array[1]='Bayan';<br> $array[2]='Reem';<br> $array[3]='Noura';<br> $result = count($array);<br> echo $result ?> </code></pre>",
     ...
     },
   {
    ...
    },
    {
        ...
        correct: 1,
        explanation:"تقوم باسترجاع جميع البينات الموجودة في الجدول  SELECT * FROM EMP"
    },
     {
     question:"أي من الجمل التالية يحتوي على خطأ",
     options: ["SELECT empid WHERE empid=56949 AND last name='SMITH'","SELECT*FROM emp WHERE empid=493945","SELECT empid FROM emp WHERE empid=493945","SELECT empid FROM emp"],
     correct:0,
     explanation:"مما يجعلها غير صحيحة FROM الجملة الخاطئة تفتقر الى "
     },
      {
     question:"ماهي نتيجة العملية الحسابية <br>Sa = 10*(6-4) ",
     options: ["30","20","10","4"],
     correct:1,
     explanation:"نقوم بحساب ما بداخل القوسين ثم ضرب الناتج في 10"
     },
     {
     question:" ماهو ناتج الدالة <br>  MOD ( 6 , 5 )",
     options: ["  36  ","  5  ","  3  ","  1  "],
     correct:3,
     explanation:" ترجع الباقي عند القسمة  MOD"
     },
     {
        question:"<bdi>x > y && a</bdi> <br>ما نتيجة الكود ",
        options:[  "x > y = true and a","x > y = false and a","يُنفذ a فقط إذا كانت x > y صحيحة","&& ترجع true إذا كانت x > y و a كلاهما true"],
        correct:0,
     ...
     },
     {
        ...
        explanation:"لان علامة || تستخدم لكلمة او"
            ...
            {
        question:"   S ماهي قيمة المتغير  <BR> بعد تنفيذ العملية التالية <br> <bdi>int  S = 19 % 4 ;</br></bdi>   ",
        options:["2","4","3","4.75"],
        correct:2,
        explanation:"لان 4*4=16 و 19-16=3"
     },
     {
        question:"ماهي مخرجات البرنامج التالي <pre><code>&lt;<?php $a = 12; <br> Echo(gettype($a));?&gt;</code></pre>",
        options:["Boolean","String","Integer","Long"],
        correct:2,
        explanation:"gettype($a) ترجع integer"
     },
     {
        question:"ماهو الخطا في الكود التالي <pre><code><?php $x=100;<br>$y=200;<br> if($x>50)&&($y>=100)<br> { echo'both value is <br> grater than 50';}?></code></pre>",
        options:["استخدام الدالة الشرطية if الصحيح استخدام Switch","استخدام الدالة الشرطية if الصحيح استخدام التكرار for","if($x>50 && $y>=100)","0"],
        correct:2,
     ...
     {
        question:"ماهي الكتابة الصح للكود التالي <br><bdi>select * from Courses <br> where mark between 90 and 100</bdi></br>",
     ...
     {
        question:"ماهو ناتج الكود التالي <br> <bdi>int i = 5;<br> int x = 15; <br> if ( i <= 5 && x >= 15)<br> {i--; System.out.print(i--);} </bdi></br>",
        options:["3","4","2","0"],
        correct:1,
        explanation:"لأن الامر <bdi>System.out.print( i-- )</bdi> يطبع القيمة اولا ثم ينقصها"
     },
     {
        question:"ماهي مخرجات البرنامج التالي <br> <pre><code>&lt;?php for (int x = 12; <BR> x > 0; x = x/2)<br>{ System.out.print(x)};&gt;?</pre></code>",
        options:["136","631","13612","12631"],
        correct:3,
        explanation:"لأن البرنامج يطبع 12 6 3 1"
     },
{
  question: "ماهي مخرجات البرنامج التالي:<br><pre><code>&lt;?php<br>$x = 1;<br>$y = 2;<br>echo \"$x + $y = \" . ($x + $y);<br?>&gt;</code></pre>",
  options: [
    "$x + $y = 3",
    "3 = 3",
    "1 + 2 = 3",  
  ...
  correct: 2,
  explanation: "في PHP، يتم تفسير المتغيرات داخل السلسلة النصية أولاً، ثم يتم جمع المتغيرين وإلحاق الناتج باستخدام العامل `.`."
},
    {
         question:"يمكن الوصول للعنصر الخامس في المصفوفة التالية باستخدام <br><bdi>int x [ ] = new int [5];</br></bdi>",
        options:["int [5]","x [5]","x [4]","x"],
        correct:2,
        explanation:"لأن المصفوفات تبدأ العد من 0 و العنصر الخامس يكون موقعة 4"
     
     }, 
     {
        question:"لتعديل درجات المتدربين في الجدول <bdi>(Students)</bdi><br> بحيث تكون 20 درجة للمتدربين الذين حصلو على اقل من 20 درجة<br>  في العمود <br> <bdi>(Degree)</bdi> نستخدم الامر",
     ...
     {
        question:"كم مرة يقوم بطباعة الكود التالي <pre><code>&lt;?php for(x = 0; x < 3; x++)<br> for(y = 0; y < 3; ++y);?&gt;</pre></code>",
        options:["4","6","8","9"],
        correct:3,
        explanation:"لأن <bdi>3</bdi> مرات للحلقة الأولى و كل مرة منها <bdi>3</bdi> مرات للحلقة الثانية 3*3=9"
     },
     {
        question:"ماهي مخرجات الكود التالي <pre><code>&lt;?php $x = 5; <br> While ($x >= 1)<br>{ echo $x; $x--;};?&gt;</code></pre>",
        options:["44321","6532","65321","54321"],
        correct:3,
        explanation:"الأرقام من 5 الى 1 بشكل تنازلي"
     },
     {
        question:"ماهي مخرجات البرنامج التالي <pre><code>&lt;?php <br> $x=array(\"a\"=>\"red\",\"b\"=>\"green\");<br> $x=array(\"c\"=>\"blue\",\"d\"=>\"yellow\");<br> var_dump($x===$y);?&gt;</br></code></pre>",
        options:["0","1","bool(true)","bool(false)"],
        correct:3,
        explanation:"لأن <bdi>$y</bdi> غير معرف فترجع <bdi>bool(false)</bdi> مع ظهور تحذير"
     },
     {
        question:"ماهي مخرجات الكود التالي <br> <bdi>Public class Test_1<br><br>{ public static void main(String[] args) <br>{ int a = 6,b = 4,c = 2; c += a + a*b; System.out.println(\"C=\"+c);}}</br></br> </bdi>",
     ...
     {
    question:"<bdi>x<br>قيمة  </bdi> عند تنفيذ العملية <br> <bdi>X=(8-5)*3^2</bdi>",
    options:["9","81","27","10"],
    correct:2,
    explanation:"نحسب ما بداخل القوس أولاً: 8-5=3، ثم 3^2=9، وأخيراً 3*9=27"
    
  },     
  {
  question: "ماهي مخرجات البرنامج التالي <pre><code dir=\"ltr\">&lt;?php\n$array[0] = 'sara';\n$array[1] = 'Bayan';\n$array[2]='Reem';\n$array[3] = 'Noura';\n$result = count($array);\necho $result;\n?&gt;</code></pre>",
  options: ["3", "4", "Error", "0"],
  correct: 1,
  explanation: "PHP تضيف العنصر <code>$array[Reem]</code> كمفتاح غير معرف، وتعتبره عنصر رابع، فيصبح عدد العناصر 4."
},
{
  question: "اذا كان <bdi>emp</bdi> يمثل جدول الموظفين و <br> <bdi>salary</bdi> يمثل الراتب فما نتيجة العبارة التالية؟<br><pre><code dir=\"ltr\">SELECT * FROM emp WHERE salary BETWEEN 985 ANDA 6000;</code></pre>",
  options: ["عرض بيانات الموظفين الذين تتراوح رواتبهم بين 985 و 6000", "عرض الموظفين برواتب أقل من 6000 فقط", "Error", "لا يتم عرض أي بيانات"],
  correct:0 ,
  explanation: "الاستعلام يعرض جميع الموظفين الذين تقع رواتبهم ضمن النطاق من 985 إلى 6000 شاملة القيمتين، باستخدام BETWEEN."
},
{
  question: "اذا كان <bdi>emp</bdi> يمثل جدول الموظفين و <br> <bdi>salary</bdi> يمثل الراتب فما نتيجة العبارة التالية؟<br><pre><code dir=\"ltr\">SELECT * FROM emp WHERE salary BETWEEN 985 ANDA 6000;</code></pre>",
  options: ["عرض بيانات الموظفين الذين تتراوح رواتبهم بين 985 و 6000", "عرض الموظفين برواتب أقل من 6000 فقط", "Error", "لا يتم عرض أي بيانات"],
...
let duration = 2 * 60 * 60 + 30 * 60; // 2h 30m = 9000 seconds
